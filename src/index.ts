...
const DOMAIN = 'https:open.ys.com'
...
var EZUIKit: Omit<EZUIKit, 'params'>  = {
...
export default class EZUIKitPlayer {
  ...
  constructor(params: Params) {
    ...
    const matchIframeUrl = (): string => {
      switch (this.opt.template) {
        case 'simple':
          var iframeUrl = DOMAIN + "/ezopen/h5/iframe?bSupporDoubleClickFull=0&url=" + this.opt.url.replace("?", "&") + "&autoplay=" + this.opt.autoplay + "&audio=" + this.opt.audio + "&accessToken=" + params.accessToken + "&templete=0" + "&id=" + this.opt.id;
          var controlsValue = "";
    ...
    var jqueryJS = this.opt.filePath + '/js/jquery.js';
      ...
      addJs(layerJs, () => {
        ...
        */
        ...
        if (matchHeaderOpt().headerContainer) {
          ...
          if (matchHeaderOpt().zoomModule) {
            ...
            enableZoomDOM.onclick = () => {
              ...
              (document.getElementById('enableZoom') as HTMLElement).setAttribute('class', 'icon hide');
              (document.getElementById('closeZoom') as HTMLElement).setAttribute('class', 'icon');
            };
            ...
            closeZoomDOM.onclick = () => {
              ...
              (document.getElementById('closeZoom') as HTMLElement).setAttribute('class', 'icon hide');
              (document.getElementById('enableZoom') as HTMLElement).setAttribute('class', 'icon');
            };
        ...
        if (matchFooterOpt().footerContainer || (this.opt.plugin as string []).indexOf('talk') !== -1) {
          var recoderCSS = this.opt.filePath + '/npm/css/recoder.css';
          var recoderJs = this.opt.filePath + '/npm/js/recoder.js';
          var recorderJs = this.opt.filePath + '/recorder.js'; // addCss()
            ...
            const apiSuccess = (data: any) => {
              ...
              if (data.code == 200) {
                ...
                if (apiResult) {
                  ...
                  var talk = "talk://" + this.opt.deviceSerial + ":0:" + this.opt.channelNo + ":cas.ys7.com:6500";
                  ...
                  if (!this.opt.isReliesReady) {
                    var adapeterJS = this.opt.filePath + '/npm/js/adapeter.js';
                    var janusJS = this.opt.filePath + '/npm/js/janus.js';
                    var ttsJS = this.opt.filePath + '/npm/js/tts.js';
          ...
          if (matchFooterOpt().footerContainer) {
            ...
            if (matchFooterOpt().broadcastModule) {
                ...
                if (data && data.length > 0) {
                  for (var i = 0; i < data.length; i++) {
                    ...
                    voiceItem.onclick = function (e) {
                      ...
                      var voiceUrl = (e.target as HTMLElement).dataset.url;
                      var time = (e.target as HTMLElement).dataset.time;
              ...
              (document.getElementById("voice-list") as HTMLElement).onscroll = function (e) {
                var sum = (this as HTMLElement).scrollHeight;
            ...
            const countTime = (type: string, start: any) => {
              ...
              const formatSeconds = (value: any) => {
                var theTime: number | string = parseInt(value); // 秒

                var middle: number = 0; // 分

                var hour = 0; // 小时

                var secondV: string | number = '00';
                var minV: string | number = '00';
                var hourV: string | number = '00';
  ...
  public play(data: any) {
    var id = 'EZUIKitPlayer-' + this.opt.id;
        var player = (document.getElementById(id) as HTMLIFrameElement).contentWindow;
  ...
  public fullScreen() {
    ...
    if ((navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i))) {
      ...
      // console.log('pc端全屏');
      var requestFullScreen = function (element: any) {
  ...
  public cancelFullScreen() {
    ...
    var player = (document.getElementById(id) as HTMLIFrameElement).contentWindow;
    if ((navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i))) {
      var width = (document.getElementById(id) as HTMLIFrameElement).width;
      var height = (document.getElementById(id) as HTMLIFrameElement).height;
      // wrapper = document.getElementById("test"),
      var wrapper = document.body;//document.body 属性返回 <body> 元素， document.documentElement 属性返回 <html> 元素。
      wrapper = document.getElementById(id) as HTMLElement;
      var style = "";
  ...
  public startTalk() {
    ...
    var apiSuccess = function (data: any) {
      if (data.code == 200) {
        var apiResult = data.data;
        if (apiResult) {
          // 临时将https转换为websocket
...
function addCss(filepath: string, callback: () => void) {
  ...
  if (!isReady) {
    var oJs: HTMLLinkElement = document.createElement('link');
    oJs.rel = 'stylesheet';
    oJs.type = 'text/css';
    oJs.href = filepath;
    oJs.onload = callback;
...
function request(url: string, method: string, params: any, header: any, success: (arg: any) => void, error: (err?: any) => any) {
  var _url = url;
  var http_request: XMLHttpRequest = new XMLHttpRequest();
